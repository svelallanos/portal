...
class Login extends Controllers
{
  ...
  public function validarLogin()
  {
    ...
    if ($seleccionar_usuario && password_verify($strPassword, $seleccionar_usuario['usuarios_password'])) {
      ...
      $_SESSION['portal']['usuario_login'] = $seleccionar_usuario['usuarios_login'];
      $_SESSION['portal']['usuario_dni'] = $seleccionar_usuario['usuarios_dni'];
      $_SESSION['portal']['usuarios_nombres'] = $seleccionar_usuario['usuarios_nombres'];
      $_SESSION['portal']['usuarios_paterno'] = $seleccionar_usuario['usuarios_paterno'];
      $_SESSION['portal']['usuarios_materno'] = $seleccionar_usuario['usuarios_materno'];
      $_SESSION['portal']['usuarios_email'] = $seleccionar_usuario['usuarios_email'];
      $_SESSION['portal']['usuarios_foto'] = $seleccionar_usuario['usuarios_foto'];
      $_SESSION['portal']['usuario_id'] = $seleccionar_usuario['usuarios_id'];
      $_SESSION['portal']['roles'] = array();
      $_SESSION['portal']['login_portal'] = true;
      $_SESSION['portal']['update_pass'] = $seleccionar_usuario['usuarios_updatepassword'];

      $roles = $usuarios->getRoles($seleccionar_usuario['usuarios_id']);
      foreach ($roles as $key => $value) {
        array_push($_SESSION['portal']['roles'], $value['roles_nombre']);
      }
